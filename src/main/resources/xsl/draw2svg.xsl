...
<xsl:template match="chord">
 ...
 -->
	...
	<xsl:variable name="swidth">220</xsl:variable>
	<xsl:variable name="sheight">220</xsl:variable>
	...
	<xsl:variable name="finalheight">
		...
		<xsl:otherwise>
			<!-- <xsl:value-of select="$topliney + $fretheight * ($maxfret + 2)"/> -->
			<xsl:value-of select="$finalwidth div (400 div $vmaxy)"/>
		</xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
	...
	<xsl:element name="svg" xmlns="http://www.w3.org/2000/svg">
		...
		<xsl:attribute name="y">0</xsl:attribute>
		 <xsl:attribute name="viewBox"><xsl:value-of select="$vx"/><xsl:text> </xsl:text><xsl:value-of select="$vy"/><xsl:text> </xsl:text><xsl:value-of select="$swidth"/><xsl:text> </xsl:text><xsl:value-of select="$sheight"/></xsl:attribute>
...
<xsl:param name="y"/>
	...
	<xsl:if test="(@fret and @fret>0) or @state='no'">
		...
		<xsl:when test="@state='opt'">
			<xsl:message>state 'opt' <xsl:value-of select="@name"/></xsl:message>	
			<!-- optional string -->
		...
		<xsl:when test="@state='no'">
			<xsl:message>state 'no' <xsl:value-of select="@name"/></xsl:message>
			<!-- disabled string -->
			<xsl:element name="line" xmlns="http://www.w3.org/2000/svg">
		...
		<xsl:otherwise>
		<xsl:message>state 'normal' <xsl:value-of select="@name"/><xsl:value-of select="@state"/></xsl:message>
			<!-- normal string -->
			<xsl:element name="circle" xmlns="http://www.w3.org/2000/svg">
				<xsl:attribute name="cx"><xsl:value-of select="$x"/></xsl:attribute>
				<xsl:attribute name="cy"><xsl:value-of select="$y"/></xsl:attribute>
